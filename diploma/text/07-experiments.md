# 6 ЭКСПЕРИМЕНТАЛЬНОЕ ИССЛЕДОВАНИЕ ПЛАТФОРМЫ
...
Таблица 6.1 — Конфигурация тестового стенда.
-->

## 6.2 Эксперимент 1 — Латентность REST API
...
Таблица 6.2 — Результаты замера латентности REST API.
...
Рисунок 6.1 — Гистограмма распределения латентности GET /tasks при 500 RPS.
Анализ: при 1000 RPS p95 превышает целевую 200 мс — необходимо горизонтальное масштабирование.
-->

## 6.3 Эксперимент 2 — Производительность ML-воркера
...
Таблица 6.3 — Throughput ML-воркера.
...
Рисунок 6.2 — Распределение времени инференса (1000 запусков).
...
## 6.4 Эксперимент 3 — Масштабирование ML-воркеров
...
Таблица 6.4 — Масштабирование ML-сервиса.
...
Рисунок 6.3 — График throughput от числа воркеров (показывает почти линейный рост).
...
## 6.5 Эксперимент 4 — Сравнение с синхронной архитектурой
...
Таблица 6.5 — Сравнение архитектур под пиковой нагрузкой.
...
Рисунок 6.4 — Распределение латентности при burst-нагрузке.
