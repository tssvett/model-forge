...
| `make cpu-inference` | Полный стек с реальной моделью на CPU (без GPU) |
...
```bash
# Требуется: NVIDIA Driver + nvidia-container-toolkit
make gpu
```

### Запуск с реальной моделью (CPU, без GPU)

```bash
# Работает на любой машине, но медленнее
make cpu-inference
```
...
- `ml-service/requirements.txt` — базовые (mock mode)
- `ml-service/requirements-cpu.txt` — PyTorch CPU + TripoSR
- `ml-service/requirements-gpu.txt` — PyTorch CUDA 11.8 + TripoSR

Общие зависимости TripoSR: omegaconf, einops, transformers, huggingface-hub, rembg, xatlas, moderngl.

Модель HuggingFace кешируется в Docker volume `huggingface-cache`.
...
│   ├── requirements.txt          # base (mock)
│   ├── requirements-cpu.txt      # CPU inference
│   ├── requirements-gpu.txt      # GPU inference
│   ├── Dockerfile                # mock (python:3.9-slim)
│   ├── Dockerfile.cpu            # CPU inference
│   └── Dockerfile.gpu            # GPU inference (nvidia/cuda)
...
│   ├── docker-compose.cpu-inference.yml  # CPU inference overlay
│   └── .env.example
